...
class ServerLogic:
    ...
    def _start(self):
        ...
        threading.Thread(target=self._handle_clients_data, daemon=True).start()
    ...
    def _handle_client_download(self, ip_addr, port, file_name, blocks, number_of_blocks, blocks_without_extra_byte):
        ...
        if ip_addr in self.users:
            ...
            print("file save size and file size: ", file_save_size, file_size)
            ...
            if number_of_blocks == len(data_blocks):  # if all data blocks are accessible (don't ask for parity blocks)
                # print("all data block accessible")
                # blocks_range = {}  # dictionary for range of each block in file
                # bytes_wrote = 0  # how many bytes wrote in file so far.
                # while bytes_wrote < file_save_size:  # while still has data to write.
                #     print('bytes wrote: ', bytes_wrote)
                #     range_list = sorted(blocks_range)
                #     ip, data = storage_files_com.msg_q.get()  # get port of a block data
                #     mac_addr = self.storage_data_com.get_mac(ip)  # get mac address of sending computer
                #     size = len(data)  # size of data
                #     bytes_wrote += len(data)  # add to length counter.
                #     # index of storage computer. (each storage computer is numbered)
                #     block_index = int(storage_computers[mac_addr])
                #     if block_index in blocks_range:  # if range for block already exists in the file
                #         start, end = blocks_range[block_index]  # get start and end of range
                #         file.index = end  # change index to the end
                #         blocks_range[block_index] = start, end + size  # change end of range in range list
                #         # change all other indexes in the range list accordingly
                #         for index in range_list[range_list.index(block_index) + 1:]:
                #             start, end = blocks_range[index]
                #             blocks_range[index] = start + size, end + size
                #     else:  # if not in list
                #         # remove first byte added while splitting the file.
                #         if block_index >= blocks_without_extra_byte:
                #             print('removed byte from block number ', block_index)
                #             data = data[1:]
                #             size -= 1
                #         first = True
                #         # find the index in the file for the block.
                #         for index in range_list:
                #             if index > block_index:
                #                 start, end = blocks_range[index]
                #                 blocks_range[block_index] = start, start + size
                #                 file.index = start
                #                 break
                #             else:
                #                 first = False
                #
                #         if first:  # if it's the first block in the file.
                #             blocks_range[block_index] = 0, size
                #             file.index = 0
                #
                #         elif block_index not in blocks_range:  # if not in range (means last block)
                #             end = blocks_range[range_list[-1]][1]
                #             file.index = end
                #             blocks_range[block_index] = end, end + size
                #
                #         range_list = sorted(blocks_range)  # resort list
                #         # change every index to match the new after block data addiction.
                #         for index in range_list[range_list.index(block_index) + 1:]:
                #             start, end = blocks_range[index]
                #             blocks_range[index] = start + size, end + size
                #
                #     self.fs.write(file, data, mode='a')  # write each block to the file (in correct index)
                #
                # self.fs.close_file(file)  # close and save the file.
                # for mac_addr in storage_computers:
                #     storage_files_com.send(self.storage_data_com.open_computers[mac_addr],
                #                            get_file_transfer_complete_msg('1').encode())
                        ...
                        if data_block not in data_block_files:
                            ...
                            for parity, contained in parities.items():  # go over every parity block
                                ...
                                if data_block in contained:  # if needed block is a part of the parity block
                                    print(f'{data_block} is in {parity} {contained}!')
                                        ...
                                        while True:
                                            ...
                                            if not blocks_data:
                                                ...
                                                if not completing_block_names and parity_data is not None:
                                                    print("parity block is data block")
...
if __name__ == '__main__':
    logic = ServerLogic(files_path=f"{os.path.join(os.getcwd(), 'server_files')}")
    while True:
        pass
